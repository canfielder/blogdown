...
states_sf_region <- states_sf %>% 
  left_join(select(state_nps_region, state_fips, region),
            by = "state_fips") %>% 
...
states_sf_region %>% 
  ggplot(aes()) +
  geom_sf(mapping = aes(fill = region), alpha = 0.75, size = 0.5, color = "#ffffff") + 
    scale_fill_manual(
      values = color_map,
    labels = c(    "Mid-Atlantic"
                 , "Midwest"
                 , "North Atlantic"
                 , "PNW & Alaska"
                 , "Rocky Mountain"
