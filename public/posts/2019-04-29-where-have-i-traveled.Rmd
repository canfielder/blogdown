...
US_state_1 <- ggplot(data = state.df_visited, 
               mapping = aes(x = long, y = lat, 
                             fill = factor(Visited), group = group))

US_state_2 <- US_state_1 + geom_polygon(color = "gray80", size = 0.05) 

US_state_3 <- US_state_2 + scale_fill_manual(values = cols) 

US_state_4 <- US_state_3 + 
  theme_map() + 
  guides(fill=FALSE)+
...
Yet something feels off about this map. Yes, I have been to all those states, but crossing a state line is not the same as exploring a state. Therefore, instead of just plotting every state I've crossed into, I think it would be interesting to plot every county I have been in.
...
```{r us_county_plot}
US_1 <- ggplot(mapping = aes(x = long, y = lat, group = group))

US_2 <- US_1 + geom_polygon(data = county.df_visited, 
                            mapping = aes(fill = factor(Visited)), 
                            color = "gray80", size = 0.05) 

US_3 <- US_2 + scale_fill_manual(values = cols) 

US_4 <- US_3 + geom_polygon(data = state.df,
                            color = "black", 
                            fill = NA, 
                            size = 0.1)
...
Well, that certainly looks a lot different than the map of just states visited. I need to get out more.
...
New_England_1 <- ggplot(data = county.df_visited_new_england, 
              aes(x = long, y = lat, fill = factor(Visited), group = group))

New_England_2 <- New_England_1 + geom_polygon(color = "gray80", size = 0.05)

New_England_3 <- New_England_2 + scale_fill_manual(values = cols)

New_England_4 <- New_England_3 + geom_polygon(data = state.df_new_england,
                            color = "black", 
                            fill = NA, 
                            size = 0.25) 

New_England_5 <- New_England_4 + theme_map() + 
  guides(fill=FALSE) +
  coord_equal()+
...
NC_1 <- ggplot(data = county.df_visited_nc, 
               aes(x = long, y = lat, fill = factor(Visited), group = group))

NC_2 <- NC_1 + geom_polygon(color = "gray80", size = 0.05)

NC_3 <- NC_2 + scale_fill_manual(values = cols)

NC_4 <- NC_3 + geom_polygon(data = state.df_nc,
                            color = "black", 
                            fill = NA, 
                            size = 0.25) 
...
This leaflet visual runs pretty slow. In the future I hope to figure out a way to get the app to run more smoothly. But this interactive visual is not just another way to visualize  counties I been to. It is perhaps more useful as a tool to determine which counties I try to visit on future trips. Roads, towns, and other places of interest are very easy to see in relation to county lines.